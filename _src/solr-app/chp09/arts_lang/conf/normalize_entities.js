...
function normalize(namespace, original) {
	if (original != null) {
		return original.toString().replace(namespace, "").replace("_", " ").replace("%20",
				" ").trim();
	} else {
		return original;
	}
...
function processAdd(cmd) {
	...
	doc.setField("NOartist_entity", normalize(ns, doc.getFieldValue("artist")));
	...
	if (doc.getFieldNames().contains('subject')) {
		...
		for (i = 0; i < subjects.size(); i++) {
			var keys = subjects.get(i).get("add").toString().split("\\[|,|\\]");
			for (j = 0; j < keys.length; j++) {
				doc.addField("subject_keyword", normalize(ns_category, keys[j]));				
			}
		}
	}
}
