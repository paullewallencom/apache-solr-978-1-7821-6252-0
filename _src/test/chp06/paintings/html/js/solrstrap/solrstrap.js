...
var SERVERROOT = 'http://localhost:8983/solr/paintings/select?q=*:*'; //SELECT endpoint
...
var FACETS = ['artist_entity','city_entity','museum_entity'];                       //facet categories
...
var HITID = 'uri'		// Name of the id field
var HITTEASER = 'comment';	// Name of field to use for teaser
var HITLINK = 'url';		// Name of field to use for link

var HL = true;
var HL_FL = 'abstract, title';
...
//when the page is loaded- do this
  ...
  (function( $ ){
	...
	if (FACETS_TITLES === undefined) return;
	else return((FACETS_TITLES && FACETS_TITLES.hasOwnProperty(facetname)) ? FACETS_TITLES[facetname] : facetname);
      });
  ...
  function get_maybe_highlit(result, i, field) 
  {
    ...
    if (HL) {
      ...
      if (hl_map !== undefined && hl_map.hasOwnProperty(field)) {
	res = hl_map[field];
      }
    }
